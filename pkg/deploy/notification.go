...
func (a *App) sendEmailNotification(ctx context.Context, user *model.User, appName string, services map[string]*deployedService, success bool) error {
	...
	if err := a.mailerClient.SendEmail(ctx, `dashboard`, `dashboard@vibioh.fr`, `Dashboard`, fmt.Sprintf(`[dashboard] Deploy of %s`, appName), recipients, notificationContent); err != nil {
